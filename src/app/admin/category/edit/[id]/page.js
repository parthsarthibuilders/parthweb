...
export default function Page({ params }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.imageFile) {
                ...
                imageUrl = uploadResponse.data.file; // Update with the new uploaded image URL
            }
    ...
    const handleCategoryChange = async (itemId) => {
        try {
            // Ensure formData.category is an array, defaulting to an empty array if not
            const updatedCategories = Array.isArray(formData.category) ? formData.category : [];
    
            // Add `id` if it doesn't already exist in the array
            const updatedCategoryList = updatedCategories.includes(id)
                ? updatedCategories // Keep the current list unchanged if `id` exists
                : [...updatedCategories, id]; // Add `id` to the list
    
            console.log("itemId:", itemId);
            console.log("id:", id);
            console.log("Updated categories before state update:", updatedCategoryList);
            ...
            const response = await axios.patch("/api/projects/update", {
                id: itemId, // Use `itemId` as the project identifier in the API call
